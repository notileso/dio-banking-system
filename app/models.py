...
class NaturalPerson(Customer):
    ...
    Pessoa Física
    """
    def __init__(self, document_number: str, name: str, birth_date: datetime, address: str):
...
class CheckingAccount(Account):
    ...
    Conta Corrente
    """
    def __init__(self, number: int, customer: Customer, balance: float = 0.0, limit: float = 100.0, limit_withdrawals: int = 3):
        self._number = number
        self._customer = customer
        self._balance = balance
        self._history = History()
    ...
    def withdraw(self, value: float) -> bool:
        number_of_withdrawals = len(
            [transaction for transaction in self.history.transactions if isinstance(transaction, Withdraw)]
        )
